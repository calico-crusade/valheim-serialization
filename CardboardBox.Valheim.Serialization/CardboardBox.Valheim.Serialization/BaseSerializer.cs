...
namespace CardboardBox.Valheim.Serialization
{
	...
	public interface IBaseSerializer<T> : IBaseSerializer
	{
		T Read(Stream stream);
		T Read(string filepath);

		void Write(Stream output, T input);
		void Write(string filepath, T input);
	}

	public abstract class BaseSerializer<T> : IBaseSerializer<T>
	{
